...
template<typename T,typename Ch,int rewrite_blocks,int MAX_BLOCK_SIZE = 1024,int MAX_SB_SIZE = 1024>
void cram_replace_test(const auto& source,const auto& dest){
    assert(source.size()%(MAX_BLOCK_SIZE/2) == 0);
    cout<<"CRAM BUILD START\n";
    CRAM<T,Ch,MAX_BLOCK_SIZE,MAX_SB_SIZE> cram(source,rewrite_blocks);
    cout<<"CRAM BUILD END\n";
    ...
    cout<<"PROGRESS: "<<0<<" CRAM USE BPC: "<<static_cast<double>(cram.use_bytes()*8)/(source.size()*2)<<'\n';
    for(int i=0;i<block_num;++i){        
        cram.replace(i,replace_data[i]);
        if((i+1)%(block_num/10)==0){
            cout<<"PROGRESS: "<<(i+1)/(block_num/10)*10<<" CRAM USE BPC: "<<static_cast<double>(cram.use_bytes()*8)/(source.size()*2)<<'\n';
        }
    }
...
int main(int argc,char **argv){
    ...
    cram_replace_test<code_t,data_t,4,1024,2048>(source,dest);
}